    ...
    void construirePlateauJeu(Stage primaryStage) {
        ...
        Timeline littleCycle = new Timeline(new KeyFrame(Duration.millis(tempo),
                event-> {animDeplacement();nbTop[0]++;
                    ...
                    if(nbTop[0] % 3 == 0){
                        int tailleVoiture = control.getVoitures().size();
                        int i = 0;

                        if(control.addVoiture(false)!=null){
                            addDessinVoiture(troupe, decalage, control.getVoitures().get(tailleVoiture));
                            i++;
                        }
                        if(control.addVoiture(true)!=null){
                            addDessinVoiture(troupe, decalage, control.getVoitures().get(tailleVoiture+i));
                        }
                    }
